create_time_variables <- function(df, time_var = abertura_lances) {
  ...
  df %>%
    mutate(ano = year(!!time_var) %>% factor(),
           ...
           inicio_bimestre = case_when(month(data) %in% 1:2 ~ str_c(as.character(year(data)), '-01-01'),
                                       month(data) %in% 3:4 ~ str_c(as.character(year(data)), '-03-01'),
                                       month(data) %in% 5:6 ~ str_c(as.character(year(data)), '-05-01'),
                                       month(data) %in% 7:8 ~ str_c(as.character(year(data)), '-07-01'),
                                       month(data) %in% 9:10 ~ str_c(as.character(year(data)), '-09-01'),
                                       month(data) %in% 11:12 ~ str_c(as.character(year(data)), '-11-01')) %>% as.Date(),
